...
graph TD
    subgraph UI [LocalRDP 控制台 - WPF 应用层]
        ...
        D["🔒 保活锁屏 (防窥黑屏 / 系统热键屏蔽 / SAS 锁定屏蔽)"]
        E["📝 运行日志 (实时诊断与导出)"]
    end

    subgraph Service [Windows 系统远程桌面服务层]
        E_svc["TermService (远程桌面核心服务)"]
    ...
    C -- "SYSTEM 计划任务 (tscon)" --> E_svc
    D -- "屏蔽系统锁屏" --> N["注册表 Policies/System"]
```
...
### 2.4 软件防窥伪锁屏保活技术 (RPA 渲染持续性方案)
为了防止用户或系统管理员直接按下物理热键 `Win+L` 锁屏导致显卡 DWM (Desktop Window Manager) 渲染服务挂起暂停，进而使后台 RPA 挂机流程失效，LocalRDP 创新设计了**“软件保活锁屏”**机制：
- **多物理显示器全覆盖**：锁屏启动时，系统自动识别并遍历物理受控端主机的所有屏幕（`System.Windows.Forms.Screen.AllScreens`），在每个屏幕上各显示一个全屏最大化且始终置顶（`Topmost=true`）的纯黑无边框窗口。其中主屏幕展示精美的时间、日期和密码解锁区域，辅助屏幕完全黑屏且隐藏鼠标指针，在物理上实现绝佳的防窥与安全阻断。
- **底层键盘钩子拦截 (Low-level Hook)**：程序在后台注入了 `WH_KEYBOARD_LL` 低级键盘 Hook 监听，吞没并拦截所有可能破坏锁屏状态的系统级切换键：包括 `Win 键`、`Alt+Tab`、`Alt+Esc`、`Ctrl+Esc`、`Alt+F4`、`Ctrl+Shift+Esc`（任务管理器），使用户无法越权脱离锁屏环境。
- **物理锁定屏蔽保护**：由于 Windows 核心安全机制，低级键盘 Hook 无法拦截 `Ctrl+Alt+Del`。为此，LocalRDP 会在锁屏激活期间临时修改当前用户的注册表策略（`DisableLockWorkstation = 1`），临时隐藏/禁用 Windows 安全界面中的“锁定”选项。解锁时会自动将其恢复，以在逻辑上和物理上实现双重安全锁定，且 DWM 显卡渲染流水线全程保持 100% 活跃渲染。
...
### 步骤 5：启动防窥保活伪锁屏 (防 Win+L 锁屏渲染失效)
1. 在 LocalRDP 右上角点击 **“🔒”** 锁屏图标。
2. 弹出的设置框中输入本次临时解锁的密码（仅对本次锁屏有效，防忘记锁死），点击 **“确认锁屏”**。
3. 锁屏启动后，主机所有屏幕都会显示全屏最上层纯黑遮罩且隐藏鼠标指针，实现物理防窥与锁定。
4. **后台运行**：此时系统底层的显卡 DWM 会话依然处于 100% 活跃渲染状态，完全不影响后台 RPA 自动化挂机或 RDP 会话的图形渲染。
5. **安全解锁**：若要解锁，直接在主屏幕的密码输入框中盲敲/输入刚才设置的密码并回车（或点击“解锁”）即可完全恢复。
...
5. **.NET Framework 4.8 环境要求**  
   本程序采用 .NET Framework 4.8 开发，**Windows 10 (1903+) / Windows 11 已默认内置该运行环境**，因此无需手动安装任何运行时，真正做到解压即用、绿色轻量。
